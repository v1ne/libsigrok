...
static int ols_convert_and_set_up_advanced_trigger(const struct sr_dev_inst *sdi, int *num_stages);
...
static int ols_convert_basic_trigger(const struct sr_dev_inst *sdi, struct ols_basic_trigger_desc *ols_trigger)
{
		...
		if (devc->raw_sample_size == num_changroups) {
			...
			if (num_changroups < 4) {
				...
				sr_spew("Expanded sample: 0x%.2hhx%.2hhx%.2hhx%.2hhx ",
					devc->raw_sample[3], devc->raw_sample[2], devc->raw_sample[1], devc->raw_sample[0]);
			}
...
process_and_forward:
	if (revents != G_IO_IN || devc->cnt_rx_raw_samples == devc->limit_samples) {
		...
		for(uint64_t i = 0; i < devc->cnt_samples/2; i++) {
			...
			memmove(&devc->sample_buf[4*i], &devc->sample_buf[4*(devc->cnt_samples-i-1)], 4);
...
static int ols_set_basic_trigger_stage(const struct ols_basic_trigger_desc *trigger_desc, struct sr_serial_dev_inst *serial, int stage)
{
	...
	RETURN_ON_ERROR(ols_send_longdata(serial, cmd, trigger_desc->trigger_mask[stage]));

	cmd = CMD_SET_BASIC_TRIGGER_VALUE0 + stage * 4;
	RETURN_ON_ERROR(ols_send_longdata(serial, cmd, trigger_desc->trigger_value[stage]));
...
SR_PRIV int ols_prepare_acquisition(const struct sr_dev_inst *sdi) {
	...
	*/
	...
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_DIVIDER, devc->cur_samplerate_divider & 0x00FFFFFF));
	...
	if (devc->max_samples > 256 * 1024) {
		RETURN_ON_ERROR(ols_send_longdata(serial, CMD_CAPTURE_READCOUNT, readcount-1));
		RETURN_ON_ERROR(ols_send_longdata(serial, CMD_CAPTURE_DELAYCOUNT, delaycount-1));
	} else {
		uint8_t arg[4];
	...
	uint32_t mask)
{
	...
	{
		if (((i ^ ((target >>  0) & 0xF)) & ((mask >>  0) & 0xF)) == 0) lutbits[0] |= lutmask;
		if (((i ^ ((target >>  4) & 0xF)) & ((mask >>  4) & 0xF)) == 0) lutbits[0] |= (lutmask << 16);
		if (((i ^ ((target >>  8) & 0xF)) & ((mask >>  8) & 0xF)) == 0) lutbits[1] |= lutmask;
		if (((i ^ ((target >> 12) & 0xF)) & ((mask >> 12) & 0xF)) == 0) lutbits[1] |= (lutmask << 16);
		if (((i ^ ((target >> 16) & 0xF)) & ((mask >> 16) & 0xF)) == 0) lutbits[2] |= lutmask;
		if (((i ^ ((target >> 20) & 0xF)) & ((mask >> 20) & 0xF)) == 0) lutbits[2] |= (lutmask << 16);
		if (((i ^ ((target >> 24) & 0xF)) & ((mask >> 24) & 0xF)) == 0) lutbits[3] |= lutmask;
		if (((i ^ ((target >> 28) & 0xF)) & ((mask >> 28) & 0xF)) == 0) lutbits[3] |= (lutmask << 16);
		lutmask <<= 1;
	}

	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_SEL, 0x20 + (num_trigger_term % 10)));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, lutbits[3]));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, lutbits[2]));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, lutbits[1]));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, lutbits[0]));
	return SR_OK;
}
...
{
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_SEL, 0x34 + (edgesel & 1)));
	...
	for (unsigned int i = 0; i < 16; i = i + 1) {
		...
		if ((i & 1) == 0)
			...
			RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, lutbits));
			lutbits = 0;
		}
	}
...
static int ols_set_advanced_trigger_timer (
	...
	uint64_t count_10ns)
{
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_SEL, 0x38 + (timersel & 1) * 2));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, count_10ns & 0xFFFFFFFF));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_SEL, 0x39 + (timersel & 1) * 2));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, count_10ns >> 32));
	return SR_OK;
}
 ...
 * Keep in mind that all primary inputs (a, edge etc.) output a pair of bits per input.
 * That's why those LUTs have 2 inputs, whereas the mid LUTs have 4 inputs. Only half
 * of the final LUT is used.
 */
...
{
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_SEL, 0x40 + (statenum * 4) + stateterm));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, finalvalue[op_final]));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, (midvalue[op_mid2] << 16) | midvalue[op_mid1]));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, (pairvalue[op_j_timer2] << 16) | pairvalue[op_i_edge2]));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, (pairvalue[op_h_range2] << 16) | pairvalue[op_fg]));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, (pairvalue[op_e_timer1] << 16) | pairvalue[op_d_edge1]));
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_WRITE, (pairvalue[op_c_range1] << 16) | pairvalue[op_ab]));
	return SR_OK;
}
	...
	uint32_t obtain_count)
{
	...
	RETURN_ON_ERROR(ols_send_longdata(serial, CMD_SET_ADVANCED_TRIG_SEL, statenum & TRIGSTATE_STATENUM_MASK));
...
{
	/*
		* Hit only when all inputs are true. Always capture for pre-trigger and acquisition.
		* Never execute the "Else" action, since we advance trigger stages implicity via hits.
		*/
...
static int ols_convert_and_set_up_advanced_trigger(const struct sr_dev_inst *sdi, gboolean* will_trigger) {
	struct sr_serial_dev_inst *serial = sdi->conn;
	struct dev_context *devc = sdi->priv;
	...
	if(devc->capture_ratio > 0) {
		/*
		 * We need to set up a timer to ensure enough samples are captured to fulfill the
		 * pre-trigger ratio. In RLE mode, this is not necessarily true. It would be possible
		 * to wait longer, to ensure that enough compressed samples are captured, but this
		 * could take ages and is probably not what the user wants.
		 */
		...
		uint64_t pretrigger_10ns_ticks =
			devc->limit_samples * effective_divider * devc->capture_ratio / 100 /* percent */;
		sr_dbg("Inserting pre-trigger delay of %" PRIu64 "0 ns", pretrigger_10ns_ticks);

		RETURN_ON_ERROR(ols_set_advanced_trigger_timer(serial, 0, pretrigger_10ns_ticks));
	...
	for (const GSList *l = trigger->stages; l; l = l->next) {
		...
		if (level_mask) {
			...
			ols_set_advanced_level_trigger(serial, num_stages_with_level_trigger, level_value, level_mask);
		...
		if (edge_rising | edge_falling) {
			if(num_stages_with_edge_trigger >= NUM_ADVANCED_EDGE_TRIGGERS) {
				sr_err("Too many edge triggers, only %d supported.", NUM_ADVANCED_EDGE_TRIGGERS);
				return SR_ERR;
			}

			ols_set_advanced_edge_trigger(serial, num_stages_with_edge_trigger, edge_rising, edge_falling, 0U);
		...
		int level_summing_input = current_level_term >= 0 ? level_stage_to_input_pair[current_level_term] : -1 ;
		int edge_summing_input = current_edge_term >= 0 ? edge_stage_to_input_pair[current_edge_term] : -1;

		if(level_summing_input >= 0)
			sum_inputs[level_summing_input] = level_stage_to_input_ab[current_level_term];
		...
		sr_spew(" Stage %d, lvl mask %.4x, edge %.4x, level term %d, edge term %d -> "
			"trigger sum %.4X %.4X %.4X %.4X %.4X %.4X %.4X %.4X",
			ols_stage, level_mask, (edge_falling | edge_rising), current_level_term, current_edge_term,
			sum_inputs[0], sum_inputs[1], sum_inputs[2], sum_inputs[3],
			sum_inputs[4], sum_inputs[5], sum_inputs[6], sum_inputs[7]);
